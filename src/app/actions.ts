...
import { addDoc, collection, doc, updateDoc, getDoc, query, where, writeBatch, getDocs } from 'firebase/firestore';
...
export async function submitBid(bidData: Omit<Bid, 'id' | 'submittedOn'>) {
    ...
    const jobSnap = await getDoc(doc(db, 'job_posts', bidData.jobId));
    const jobDoc = jobSnap.data() as Job;
